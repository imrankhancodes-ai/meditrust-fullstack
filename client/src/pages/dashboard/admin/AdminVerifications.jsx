...
import { Spinner, ErrorState, Button, Badge } from "../../../components/ui/ui";
...
export default function AdminVerifications() {
  ...
  return (
    <div>
      <h1 className="text-xl font-extrabold">Verifications</h1>
      <div className="mt-3 flex gap-2">
        {(["doctors", "pathologists"]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`btn-press rounded-full px-4 py-1.5 text-sm font-bold capitalize ${tab === t ? "bg-teal-700 text-white" : "bg-white text-slate-600 shadow-sm"}`}
          >
            {t}
          </button>
        ))}
...
function VerifyList({ loading, error, retry, items, pendingCount, kind, onVerify, busy }) {
  ...
  return (
    <div className="mt-4">
      <p className="text-sm text-slate-500">{pendingCount} pending request(s)</p>
      <div className="mt-2 space-y-3">
        {sorted.map((x) => (
          <div key={x._id} className="rounded-2xl bg-white p-4 shadow-sm">
        ...
        {(!items || items.length === 0) && (
          <p className="rounded-2xl bg-white p-8 text-center text-sm text-slate-500">No requests.</p>
        )}
      </div>
    </div>
  );
